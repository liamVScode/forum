...
public interface ChatRepo extends JpaRepository<Chat, Long> {
    ...
    @Query("SELECT c FROM Chat c JOIN c.members m WHERE :userId IN (SELECT m.user.userId FROM c.members m) " +
            "AND :otherUserId IN (SELECT m.user.userId FROM c.members m) AND SIZE(c.members) = 2" +
    ...
    @Query("SELECT c FROM Chat c JOIN c.members m WHERE m.user.userId = :userId")
